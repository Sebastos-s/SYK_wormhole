...
def PFD(A, B):
    ...
    '''
    return 0.5*((1/(A+B)) - (1/(B-A)))


def custom_eta(omega,eta_left, eta_right, loc):
    '''
    Custom eta function with big increase after loc -- 
    '''

    idx = (np.abs(omega - loc)).argmin()
    eta = np.zeros_like(omega)
    eta = eta_left + eta_right/(1 + np.exp(-500*(np.abs(omega) - loc))) 
